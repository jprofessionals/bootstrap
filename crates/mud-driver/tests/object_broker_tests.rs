...
fn volatile_results_are_not_cached() {
    ...
    broker.cache_result(42, "get_hp", Value::Int(100), CachePolicy::Volatile);
...
fn invalidate_by_object_ids_clears_cache() {
    ...
    assert!(
        broker.check_cache(10, "get_name").is_none(),
        "object 10 should be invalidated"
    );
    assert!(
        broker.check_cache(30, "get_name").is_none(),
        "object 30 should be invalidated"
    );
...
fn invalidate_with_empty_list_is_noop() {
    ...
    assert!(
        broker.check_cache(10, "get_name").is_some(),
        "empty invalidation should be a no-op"
    );
}
...
fn multiple_methods_on_same_object_cached_independently() {
    ...
    let name = broker
        .check_cache(42, "get_name")
        .expect("get_name should be cached");
    let weight = broker
        .check_cache(42, "get_weight")
        .expect("get_weight should be cached");
    let value = broker
        .check_cache(42, "get_value")
        .expect("get_value should be cached");
